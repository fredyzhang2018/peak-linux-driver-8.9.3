...
#ifdef DEBUG
	pr_info(DEVICE_NAME ": %s(%s CAN%u, cmd=%u)\n",
		__func__, dev->adapter->name, dev->nChannel+1, cmd);
#endif
...
#ifdef DEBUG
	pr_info(DEVICE_NAME ": %s(CAN%u, cmd=%u): returns %d\n",
		__func__, dev->nChannel+1, cmd, err);
#endif
	...
	if (pcan_parse_input_idle(dev_priv->pcWriteBuffer)) {
		...
		if (pcan_parse_input_message(dev_priv->pcWriteBuffer,
			...
			DPRINTK(KERN_DEBUG "%s: *** 0x%08x 0x%02x %d . ",
				DEVICE_NAME, tx.msg.id, tx.msg.flags,
				tx.msg.data_len);

			while (i++ < tx.msg.data_len)
				DPRINTK(KERN_DEBUG "0x%02x ", tx.msg.data[i]);
