...
static int pcan_usb_stop(struct pcandev *dev)
{
	...
	pcan_usb_kill_urb(&u->write_data);
#endif
	DPRINTK(KERN_DEBUG "%s: have still %d+%d active URBs on interface\n",
	        DEVICE_NAME, atomic_read(&usb_if->r_active_urbs),
...
#ifdef DEBUG
	pr_info(DEVICE_NAME ": %s(), minor = %d. (nOpenPaths=%d)\n",
...
#ifdef DEBUG
	pr_debug(DEVICE_NAME ": %s(): bNumEndpoints=%d\n",
